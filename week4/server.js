  ...
  if (req.url === "/api/credit-package" && req.method === "GET") {
    ...
    req.on("end", async () => {
      try {
        ...
        const { name, price, credit_amount } = data;
        const newPackage = packageRepo.create({ name, price, credit_amount });
    ...
    try {
      ...
      const result = await packageRepo.delete(packageId);
    ...
    try {
      const skill = await skillRepo.find({
        select: ["id", "name"]
      });
      res.writeHead(200, headers);
      res.write(JSON.stringify({
